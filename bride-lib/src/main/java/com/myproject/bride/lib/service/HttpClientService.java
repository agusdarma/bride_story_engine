...
public class HttpClientService {
	...
	public String getTransactionStatus(String orderId) throws BrideEngineException, IOException {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				throw new BrideEngineException(BrideEngineException.NE_ERROR_RESPONSE_HOST);
			}  // end if statusCode != 200
		} catch (BrideEngineException me) {
			throw me;
		}catch (Exception e) {
			if (httpGet != null)
				httpGet.abort();
            LOG.warn("Unknown Error", e);
            throw new BrideEngineException(BrideEngineException.ENGINE_UNKNOWN_ERROR);
        }   finally {
	    	if (httpGet != null)
	...
	public String chargeMidtransPost(String body) throws BrideEngineException, IOException {
